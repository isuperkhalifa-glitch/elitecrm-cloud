...
-- Run once in Supabase SQL Editor after deploying the application patch.
...
begin
  ...
  if target.id is null then
    update public.leads
    set registration_status = 'not_registered',
  ...
  update public.leads
  set company_id = target.company_id,
      course_id = target.course_id,
      registration_status = target.status,
      payment_status = target.payment_status,
      registration_amount = target.list_price,
      discount_amount = target.discount_amount,
      final_amount = target.final_price,
      discount_code = target.discount_code,
      paid_amount = target.paid_amount,
      status = case
        when target.payment_status = 'paid' then 'paid'
        when coalesce(status, '') = 'paid' then 'interested'
        else status
      end,
      customer_status = case
        when target.payment_status = 'paid' then 'paid'
        when coalesce(customer_status, '') = 'paid' then 'interested'
